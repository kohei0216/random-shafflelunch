(function($) {
	var Roulette = function(options) {
		...
		var displayResult = function() {
			if (stopImageNumber != undefined) {
				if (slotNum == 1) {
					setTimeout(function(){ $('div.group:last-of-type').append(`<p>${name_01[p.stopImageNumber]}</p>`);},1200);
				} else if (slotNum == 2) {
					setTimeout(function(){ $('div.group:last-of-type').append(`<p>${name_02[p.stopImageNumber]}</p>`);},1200);
				} else if (slotNum == 3) {
					setTimeout(function(){ $('div.group:last-of-type').append(`<p>${name_03[p.stopImageNumber]}</p>`);},1200);
				} else if (slotNum == 4) {
					setTimeout(function(){ $('div.group:last-of-type').append(`<p>${name_04[p.stopImageNumber]}</p>`);},1200);
				}
			} else if (stopImageNumber == undefined) {
		...
		var roll = function() {
			...
			if (p.maxDistance && p.distance >= p.maxDistance) {
				p.isStop = true;
				reset();
				p.$rouletteTarget.find('img').eq(p.stopImageNumber).addClass("active-img");
		...
		var start = function(stopImageNumber) {
			p.playCount++;
...
*/
		...
		var stop = function(option, slotNum) {
/*
			if (!p.isSlowdown) {
				if (option) {
